...
void FrsManagerImplementation::promotePlayer(CreatureObject* player) {
	...
	info("Player: " + player->getFirstName() + " has enough xp to rank, promoting to rank: [" + String::valueOf(newRank) + "]",true);
...
void FrsManagerImplementation::adjustFrsExperience(CreatureObject* player, int amount) {
	...
	if (amount > 0) {
		info("FRS: Awarding XP to [" + player->getFirstName()  + "]",true);
		...
		info("FRS: Player [" + player->getFirstName() + "] recognized as rank [" + String::valueOf(rank) + "]",true);
		...
		info("FRS: Removing XP from [" + player->getFirstName()  + "]",true);
...
int FrsManagerImplementation::calculatePvpExperienceChange(CreatureObject* attacker, CreatureObject* victim, float contribution, bool isVictim) {
	...
	info("Entered FRS XP calculations for attacker: " + attacker->getFirstName() + " Defender: " +  victim->getFirstName() + "Contribution values is:" + String::valueOf(contribution),true);
	...
	info("ratingDiff calulated to:" + String::valueOf(ratingDiff) + " XpChange Calulated to:" + String::valueOf(xpChange),true);
...
int FrsManagerImplementation::getBaseExperienceGain(PlayerObject* playerGhost, PlayerObject* opponentGhost, bool playerWon) {
	...
	info("FRS: PlayerCouncil = [" +  String::valueOf(playerCouncil) + "] PlayerRank[" + String::valueOf(playerRank) +"]   Opponent Rank[" + String::valueOf(opponentRank) + "]",true);
