...
bool PlayerCreationManager::createCharacter(ClientCreateCharacterCallback* callback) const {
			...
			try {
				...
				if (accountPermissionLevel < 9) {
					...
					if (lastCreatedCharacter.containsKey(accID)) {
						...
						if (lastCreatedTime.miliDifference() < 36000) {
							ErrorMessage* errMsg = new ErrorMessage("Create Error", "You are only permitted to create one character per hour. Repeat attempts prior to 1 hour elapsing will reset the timer.", 0x0);
							client->sendMessage(errMsg);
	...
	//Join Buff chat room
	ghost->addChatRoom(chatManager->getbuffRoom()->getRoomID());

	ManagedReference<SuiMessageBox*> box = new SuiMessageBox(playerCreature, SuiWindowType::NONE);
	box->setPromptTitle("A new Infinitian is born!");
	box->setPromptText("Welcome to Infinity. Join the Infinity Galaxy chat room to speak with other players and check out the player-made Cities spread throughout the Galaxy. Any questions? Just ask in our Discord server. Have fun!");

	String playerName = playerCreature->getFirstName();
	StringBuffer zBroadcast;
	zBroadcast << "\\#0D9DDD" << playerName << " \\#FFFFFF has arrived. Welcome to Infinity!";
