...
void PlayerManagerImplementation::killPlayer(TangibleObject* attacker, CreatureObject* player, int typeofdeath, bool isCombatAction) {
	...
	//BH PVP system adapted from rising
	if (attacker->isPlayerCreature() && attacker != player){
		...
		box->setPromptText("Place a bounty on your killer! Enter an amount between 25k and 2.5m credits. The Bounty Hunter Guild will take 20% for their fees and your target will be added to our boards immediately.");
		box->setMaxInputSize(128);
		box->setCancelButton(true, "@no");
		box->setOkButton(true, "@yes");
		box->setUsingObject(attacker);
		box->setForceCloseDistance(1024.f);
...
bool PlayerManagerImplementation::doRocketBoots(CreatureObject* player, float hamModifier, float cooldownModifier) {
	...
	if (player->isRidingMount()) {
		player->sendSystemMessage("You cannot use your rocketboots while mounted."); // You cannot burst-run while mounted on a creature or vehicle.
		return false;
	}

	if (player->hasBuff(STRING_HASHCODE("gallop")) || player->hasBuff(STRING_HASHCODE("burstrun")) || player->hasBuff(STRING_HASHCODE("retreat"))) {
		player->sendSystemMessage("You cant use your rocketboots right now");
		return false;
	}
	...
	if(player->hasBuff(forceRun1CRC) || player->hasBuff(forceRun2CRC) || player->hasBuff(forceRun3CRC)) {
		player->sendSystemMessage("Increasing your speed further is impossible");
		return false;
	}
	...
	if (!player->checkCooldownRecovery("rocketboots")) {
		player->sendSystemMessage("Your rocketboots have not yet recharged");
		return false;
	}
	...
	int newCooldown = (int) cooldown;
 	player->sendSystemMessage("You activate your rocket boots, gaining a burst of speed.");
...
bool PlayerManagerImplementation::doDash(CreatureObject* player, float hamModifier, float cooldownModifier) {
	...
	if (player->isRidingMount()) {
		player->sendSystemMessage("@cbt_spam:no_burst"); // You cannot Dash while mounted on a creature or vehicle.
		return false;
	}

	if (player->hasBuff(STRING_HASHCODE("burstrun")) || player->hasBuff(STRING_HASHCODE("gallop")) || player->hasBuff(STRING_HASHCODE("rocketboots")) ) {
...
bool PlayerManagerImplementation::doBurstRun(CreatureObject* player, float hamModifier, float cooldownModifier) {
	...
	if (player->hasBuff(STRING_HASHCODE("burstrun")) || player->hasBuff(STRING_HASHCODE("gallop")) || player->hasBuff(STRING_HASHCODE("rocketboots")) || player->hasBuff(STRING_HASHCODE("retreat")) ) {
		player->sendSystemMessage("@combat_effects:burst_run_no"); // You cannot burst run right now.
		return false;
	}
...
void PlayerManagerImplementation::gmenhanceCharacter(CreatureObject* player, int buffAmount) {
	...
	message = message && doEnhanceCharacter(0x98321369, player, buffAmount, 10800, BuffType::MEDICAL, 0); // medical_enhance_health
	message = message && doEnhanceCharacter(0x815D85C5, player, buffAmount, 10800, BuffType::MEDICAL, 1); // medical_enhance_strength
	message = message && doEnhanceCharacter(0x7F86D2C6, player, buffAmount, 10800, BuffType::MEDICAL, 2); // medical_enhance_constitution
	message = message && doEnhanceCharacter(0x4BF616E2, player, buffAmount, 10800, BuffType::MEDICAL, 3); // medical_enhance_action
	message = message && doEnhanceCharacter(0x71B5C842, player, buffAmount, 10800, BuffType::MEDICAL, 4); // medical_enhance_quickness
	message = message && doEnhanceCharacter(0xED0040D9, player, buffAmount, 10800, BuffType::MEDICAL, 5); // medical_enhance_stamina

	message = message && doEnhanceCharacter(0x11C1772E, player, entbuffAmount, 10800, BuffType::PERFORMANCE, 6); // performance_enhance_dance_mind
	message = message && doEnhanceCharacter(0x2E77F586, player, entbuffAmount, 10800, BuffType::PERFORMANCE, 7); // performance_enhance_music_focus
	message = message && doEnhanceCharacter(0x3EC6FCB6, player, entbuffAmount, 10800, BuffType::PERFORMANCE, 8); // performance_enhance_music_willpower
...
int PlayerManagerImplementation::calculateAttackers(CreatureObject* victim, CreatureObject* killer, ThreatMap* threatMap){

	int count = 1;
	int groupId = 0;

	//add everyone on the victim's threat list to the list, then iterate over the attackers group members that in range and count them as well making sure not to duplicate
	if (killer->isGrouped())
		groupId = killer->getGroupID();

	PlayerObject* defenderGhost = victim->getPlayerObject();
        Vector3 pos(victim->getWorldPositionX(), victim->getWorldPositionY(), 0);
	...
	for (int i = 0; i < threatMap->size(); ++i) { //we should have been passed the victim's threatmap here as initial arguments for the function
		...
		if (entry == NULL || attacker == NULL || attacker == killer || attacker == victim || !attacker->isPlayerCreature())
			continue;

		PlayerObject* attackerGhost = attacker->getPlayerObject();

		info("FRS: calculateAttackers: Info on participant: [" + attacker->getFirstName() + "]",true);

		if (attacker->isGrouped()){
			if (attacker->getGroupID() == groupId){
				info("FRS: calculateAttackers: Participant is in killer or defender's group, not counting as part of threatmap",true);
				continue; //dont double count group members
			}

			if (attacker->getZone() != victim->getZone()){
			info("FRS: calculateAttackers: " + attacker->getFirstName() + " is not in the same zone as the victim, skipping",true);
			continue;
			}
		        if (CombatManager::instance()->areInDuel(attacker, victim)){
		                info("FRS: calculateAttackers: Member and victim are in a duel, no FRS granted",true);
				continue;
		        }
		}
		    //    if (CombatManager::instance()->areInDuel(attacker, victim)){
		      //          error("Member and victim are in a duel, no FRS granted");
				//continue;
		      //  }
		...
		if (!allowSameAccountPvpRatingCredit && defenderGhost->getAccountID() == attackerGhost->getAccountID()){
			info("FRS: calculateAttackers ThreatMap: Participant is another character on the same account, disallowing FRS Credit",true);
			continue;
		}

		if (isSameFaction(victim, attacker)){
			info("FRS: calculateAttackers ThreatMap: Participant is same faction, skipping",true);
			continue; //Cant be same faction
		}

		if (pos.distanceTo(attacker->getWorldPosition()) > 256){
			info("FRS: calculateAttackers ThreatMap: Participant is out of range: [" + String::valueOf(pos.distanceTo(victim->getWorldPosition())) + "]",true);
			continue;
		}
		info("FRS: calculateAttackers ThreatMap: Participant added to threatmap total.",true);
		count++;
		}
		...
		if (group == NULL){
			info("FRS: calculateAttackers Killer/Defender is ungrouped, bypassing group check",true);
			return count;
		}
		...
		for (int i = 0; i < group->getGroupSize(); i++) {
			...
			if (member == NULL || memberGhost == NULL || member == killer || !member->isPlayerCreature())
			continue; //dont count the killer twice, dont count pets

			if (member->getFactionStatus() != FactionStatus::OVERT && !memberGhost->hasBhTef()){
				info("FRS: calculateAttackers GroupCheck: Participant is not overt or a BH with an active mission, not counting.",true);
			continue; //we're only interested in overts or bounty hunters in the group that were missed on the threatmap check
			}

			if (member->getZone() != victim->getZone()){
			info("FRS: calculateAttackers: " + member->getFirstName() + " is not in the same zone as the victim, skipping",true);
			continue;
			}

		        if (CombatManager::instance()->areInDuel(member, victim)){
		                info("FRS: calculateAttackers member and victim are in a duel, no FRS granted",true);
				continue;
		        }


			if (isSameFaction(victim, member)){
				info("FRS: calculateAttackers GroupCheck: Participant is same faction, skipping",true);
				continue; //Cant be same faction
			}

			if (pos.distanceTo(member->getWorldPosition()) > 256){
				info("FRS: calculateAttackers GroupCheck: Participant is out of range for credit - Distance: [" + String::valueOf(pos.distanceTo(member->getWorldPosition()))  + "]",true);
...
void PlayerManagerImplementation::awardPvpProgress(TangibleObject* killer, CreatureObject* destructedObject, bool performedDeathBlow = false) {
	...
	Reference<ThreatMap*> copyAttackerThreatMap = new ThreatMap(*attackerThreatMap);




	if (!destructedObject->isRebel() && !destructedObject->isImperial() && !killedGhost->hasBhTef()){
		info("FRS: PVP Victim is not factionally aligned or a BH, no FRS Granted",true);
		return; //The victim is not factionally aligned and is therefor granted no PVP progress
	}

	//Lets see how many were involved.
	info("FRS: Involved Attackers for the death of: " + destructedObject->getFirstName(),true);
	int countAttackers = calculateAttackers(destructedObject,killerCreature,attackerThreatMap);
	info("FRS: Involved Defenders for the death of: " + destructedObject->getFirstName(),true);
	int countDefenders = calculateAttackers(killerCreature,destructedObject,victimThreatMap);

	info("FRS: New FRS Kill [" + String::valueOf(countAttackers) + "] Attackers, [" + String::valueOf(countDefenders) + "] Defenders",true);
	...
	info("FRS: Award: Base set to 1k",true);
	if (destructedObject->hasSkill("force_title_jedi_rank_02")){
		award += 1500; //2500 base for padawans and hybrids
		info("Award: +Padawan Bonus = " + String::valueOf(award),true);
	}

  	if (destructedObject->hasSkill("combat_bountyhunter_investigation_03")){
  		award += 2500; //2500 bonus for bounty hunter
  		info("Award: +Bounty Hunter Bonus = " + String::valueOf(award),true);
  	}

	if (destructedObject->hasSkill("force_title_jedi_rank_03")){
		award += 2500; //5000 base for knights
		info("Award: +Knight Bonus = " + String::valueOf(award),true);
	}

	int victimRank = getRank(destructedObject);

	award += victimRank * 500; //500 bonus per rank
	info("FRS: Award: +Rank Bonus" + String::valueOf(award));

	if (countDefenders < 1 || countAttackers < 1){
		error("FRS: Defender or attacker counter is below 1, this should not be possible!");
		return; //Wtf?
	}

	float sizeRatio = Math::max(1,countAttackers/countDefenders);
	info("FRS:  Award: ratio of the difference in group size calculated to: " + String::valueOf(sizeRatio),true);
	int awardBase = award * .25; //The very minimum they will get for the kill
	award *= .75; //reduced by 25% to account for the 25% base that is guaranteed

	int modifiedAward = awardBase + (award/sizeRatio); //guaranteed base + bonus or negative depending on group ratio size

	info("FRS: Award: modfiiedAward = " + String::valueOf(modifiedAward),true);

	ManagedReference<GroupObject*> group = killerCreature->getGroup();
	int finalAward = modifiedAward;
	if (group != NULL){//apply final penalty of up to 50% based on group size
		info("FRS: Killer is grouped, adjusting down for group members, maximum penalty of 1/2",true);
		float gSize = Math::min(10,group->getGroupSize());// get the group size, cap it at 10

		info("FRS: Award: groupSize value = " + String::valueOf(gSize),true);

		float groupPenalty = (gSize * 5)/100;
		info("FRS: Award: Group Size Penalty = " + String::valueOf(groupPenalty),true);
		finalAward = finalAward * groupPenalty;
		info("FRS: Award: After Group Size Penalty = " + String::valueOf(finalAward),true);
		if (finalAward < (modifiedAward/2)){
		info("FRS: Final payout calculated to less than half original value, increasing to half payout) New Value = " + String::valueOf(finalAward),true);
			finalAward = modifiedAward/2; //group size penalty will never be more than 50% of expected
		}
	}

	if (sizeRatio < 1){ //Killer won against overwhelming odds, they are granted a bonus of 25% for each additional attacker
		info("FRS: Victim Won against a larger attacking force, bonus applied.",true);
		finalAward += ((modifiedAward * .25)/sizeRatio);
		info("FRS: Award: +OverWhelming Bonus" + String::valueOf(finalAward),true);
	}

	bool awardAllowed = true;

	if (!allowSameAccountPvpRatingCredit && ghost->getAccountID() == killedGhost->getAccountID()){
		info("FRS: No killer award allowed, same account",true);
		awardAllowed = false;
	}

	if (killerCreature->getFactionStatus() != FactionStatus::OVERT){
		info("FRS: Killer is not Overt, no FRS granted",true);
		awardAllowed = false;
	}

	if (CombatManager::instance()->areInDuel(killerCreature, destructedObject)){
                info("Killer and victim are in a duel, no FRS granted",true);
                awardAllowed = false;
	}

	if ((killerCreature->hasSkill("force_rank_light_novice") || killerCreature->hasSkill("force_rank_dark_novice"))){
		//This prevents tactics like using a BH to lock a teammate out of the greater GCW fights, this is a valid tactic and still works, but the rest of your teammates
	   	//not geting FRS for assisting the bounty hunter should help to dissincentivize that behvaior.
		if (killedGhost->hasBhTef() && !destructedObject->hasSkill("combat_bountyhunter_investigation_03")){
			info("FRS: <Victim has recently, or is actively being hunted by a BH, no FRS is awarded>",true);
		...
		else if (awardAllowed){
			info("FRS: Award: Paying FRS to Killer, final Award:" + String::valueOf(finalAward),true);
			frsManager->adjustFrsExperience(killerCreature, finalAward);
		}
	}

	Vector3 pos(destructedObject->getWorldPositionX(), destructedObject->getWorldPositionY(), 0);

		if (group != NULL) {
			info("FRS: Award: Killer is grouped, paying out FRS to all eligible",true);
			for (int i = 0; i < group->getGroupSize(); i++) {
				CreatureObject* member = group->getGroupMember(i);
				PlayerObject* memberGhost = member->getPlayerObject();


				if (member->getZone() != destructedObject->getZone()){
				info("FRS: " + member->getFirstName() + " is not in the same zone as the victim, skipping",true);
				continue;
				}

				info("FRS: Award: Start Check for Participant [" + member->getFirstName() +  "]",true);

				if (member == NULL || memberGhost == NULL || member == killerCreature || !member->isPlayerCreature() || member == destructedObject){
					info("FRS: Participant is Killer or Victim, skipping: " + member->getFirstName(),true);
				continue; //dont count the killer twice, dont count pets, dont count the victim if they some how managed to hurt themselves
			      ...
			      if (CombatManager::instance()->areInDuel(member, destructedObject)){
			            info("FRS: Member and victim are in a duel, no FRS granted, skipping: " + member->getFirstName(),true);
					continue;
			      }

				if (member->getFactionStatus() != FactionStatus::OVERT){
					info("FRS: Participant is not Overt, skipping: " + member->getFirstName(),true);
				continue; //we're only interested in overts
 				}

				if (isSameFaction(destructedObject, member)){
					info("FRS: Participant is same faction, skipping, " + member->getFirstName(),true);
 					continue; //Cant be same faction
				}

				if (pos.distanceTo(member->getWorldPosition()) > 256){
					info("FRS: " + member->getFirstName() + "is Out of range of " + destructedObject->getFirstName() + ", Distance: [" + String::valueOf(pos.distanceTo(member->getWorldPosition())) + "] meters",true);
				...
				if (!killedGhost->hasBhTef() || destructedObject->hasSkill("combat_bountyhunter_investigation_03")){
					...
					if (member->hasSkill("force_rank_light_novice") || member->hasSkill("force_rank_dark_novice")){
						Locker mLocker(member);
						info("FRS: Award: Granting FRS to Participant [" + String::valueOf(finalAward) + "]",true);
						frsManager->adjustFrsExperience(member, finalAward); //Award the FRS Xp
					}
	...
	int deductionXp = -1 * (modifiedAward * .65); //65% of the FRS Paid out is charged to the defender.
	info("FRS: Award: deduction XP calculated to .65 of modifiedAward = " + String::valueOf(deductionXp),true);

	if (sizeRatio >= 4){
	deductionXp = 0;
        info("FRS: Award: Overhwelming odds protection activated, no FRS XP Lost",true);
	}
	...
	info("PVP: Killer " + killerName + " coordinates (x=" + kX + ", y=" + kY + ", z=" + kZ + ").  Victim " + playerName + " coordinates (x=" + pX + ", y=" + pY + ", z=" + pZ + ").",true);
	...
	if (killerCreature->hasSkill("force_rank_light_novice") && destructedObject->hasSkill("force_rank_dark_novice")) {
		zBroadcast << "Jedi " << killerName << " has defeated" << " Dark Jedi " <<  playerName << " in the FRS";

	} else if (killerCreature->hasSkill("force_rank_dark_novice") && destructedObject->hasSkill("force_rank_light_novice")) {
		zBroadcast << "Dark Jedi " << killerName << " has defeated " <<  playerName << " in the FRS";
	}
	else {
		zBroadcast << playerName << " was slain in the GCW by " <<  killerName;
	}

	ghost->getZoneServer()->getChatManager()->broadcastGalaxy(NULL, zBroadcast.toString());

	//TODO: Check if the combantant has GCW skills and dock GCW xp accordingly
}
